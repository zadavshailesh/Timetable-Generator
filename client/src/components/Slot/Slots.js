...
class Slots extends Component {
  ...
  async generator(slot,teacher, classes) {
    let slots = [];
    let numPeriods = 0;
    slot ? numPeriods = slot.sunday+slot.monday+slot.tuesday+ slot.wednesday+ slot.thrusday+slot.friday : numPeriods = 0;
    ...
    const result = await generate(
      slots,
      [slot.sunday ? slot.sunday : 0, slot.monday ? slot.monday : 0, slot.tuesday ? slot.tuesday : 0,
        slot.wednesday ? slot.wednesday : 0, slot.thrusday ? slot.thrusday : 0, slot.friday ? slot.friday : 0],
      teacher.teachersName,
      classes.classAndsec
    );
    const maxm = Math.max(
      slot.sunday, slot.monday, slot.tuesday, slot.wednesday, slot.thrusday, slot.friday);
      ...
      <div className="mt-5">
        <Form onSubmit={this.onSubmit}>
          <Row>
            ...
            <Col md={{ size: 3, offset: 0.5 }}>
              <Label>No. Of Class (weekly)</Label>
              <Input
                type="number"
                name="numLectures"
